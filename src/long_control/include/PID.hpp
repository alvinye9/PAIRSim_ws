...
namespace control
{
  ...
  public:
    ...
    /// @param[in] iMax is a bound on the maximum integrated error (+/-). Defaults to 1e8.
    /// @param[in] cmdMax is an upper bound on the output command. Defaults to 1e8.
    /// @param[in] cmdMin is a lower bound on the output command. Defaults to -1e8.
    PID(const double &p = 0.0, const double &i = 0.0, const double &d = 0.0, const double &dt = 0.01,
      const double &iMax = 1e8, const double &cmdMax = 1e8, const double &cmdMin = -1e8);
    ...
    /// @param min is the minimum command output. Must be less than or equal to zero.
    /// @param max is the maximum command output. Must be greater than zero.
    /// @return a pair of bools. First is true if min is set correctly, second is true if max is set correctly.
  ...
  private:
    ...
    /// @brief The time step the controller is running at. Must be greater than zero.
    double dt_ = 0.01;
    ...
    double eOld_ = 0.0;
    /// @brief Current integrated value of the control error used in the PID control algorithm
    double eInt_ = 0.0;
  }; // End class PID
