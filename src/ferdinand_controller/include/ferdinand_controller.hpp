...
#include "blackandgold_msgs/msg/stanley_adrc_debug.hpp"
...
class StanleyADRC : public rclcpp::Node {
 public:
  StanleyADRC() : Node("StanleyAdrcNode") {
    ...
    this->declare_parameter("K1sat", 3.14/4);
    this->declare_parameter("K1d", 0.0);
    this->declare_parameter("K2", 0.0);
    this->declare_parameter("K2d", 0.0);
    this->declare_parameter("K2sat", 0.0);
    this->declare_parameter("K3", 0.0);
    this->declare_parameter("K3sat", 0.0);
    this->declare_parameter("K4", 0.0);
    this->declare_parameter("K4_curv", 0.0);
    this->declare_parameter("cascade_mod",10);
    ...
    // Publishers
    pubLookaheadError_ = this->create_publisher<std_msgs::msg::Float64>("/control/ferdinand_controller/lookahead_error", 10);
    pubLatError_ = this->create_publisher<std_msgs::msg::Float64>("/control/ferdinand_controller/lateral_error", 10);
    pubErrorReporter_ =
        this->create_publisher<blackandgold_msgs::msg::ErrorReport>("/vehicle/errors", 5);
    controlDebugSignals_ = this->create_publisher<blackandgold_msgs::msg::StanleyADRCDebug>("/control/ferdinand_controller/debug_signals", 10);
    pubYawRef = this->create_publisher<std_msgs::msg::Float32>("/control/ferdinand_controller/desired_heading", 10);
    pubCurvature = this->create_publisher<std_msgs::msg::Float32>("/control/ferdinand_controller/curvature", 10);
    ...
    pubControlCmd_ = this->create_publisher<deep_orange_msgs::msg::JoystickCommand>("/control/ferdinand_controller/accel_command", rclcpp::SensorDataQoS());
    ...
    pubDesiredAccel_ = this->create_publisher<std_msgs::msg::Float32>("/control/desired_acceleration", 1); // TODO is this needed?

    pubHeadingLookaheadMarker_ = this->create_publisher<visualization_msgs::msg::Marker>("/control/ferdinand_controller/heading_lookahead_point_marker",1);
    pubCurvatureLookaheadMarker_ = this->create_publisher<visualization_msgs::msg::Marker>("/control/ferdinand_controller/curvature_lookahead_point_marker",1);

    sleep(3);

    RCLCPP_INFO(this->get_logger(), "Starting Stanley ADRC controller");

    // Subscribers
    subPath_ = this->create_subscription<nav_msgs::msg::Path>(
        "/planning/front_path/offset_path", 1,
        std::bind(&StanleyADRC::receivePath, this, std::placeholders::_1));
    subWheelVelocity_ = this->create_subscription<raptor_dbw_msgs::msg::WheelSpeedReport>( "/raptor_dbw_interface/wheel_speed_report", qos,std::bind(&StanleyADRC::receiveWheelVelocity, this, std::placeholders::_1));
    
    subVelocity_ = this->create_subscription<std_msgs::msg::Float32>(
        "/localization/vehicle_speed", qos,
        std::bind(&StanleyADRC::receiveVelocity, this, std::placeholders::_1));
    subDesiredVelocity_ = this->create_subscription<std_msgs::msg::Float32>(
      "/planning/desired_velocity", qos,
      std::bind(&StanleyADRC::receiveDesiredVelocity, this, std::placeholders::_1));
    subPathCurvature_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
      "/planning/front_path/curvature", qos, std::bind(&StanleyADRC::calculateCurvatureVelocity, this, std::placeholders::_1));
     
    subGlobalOdom_ = this->create_subscription<nav_msgs::msg::Odometry>(
      "/odometry/global_filtered", qos, std::bind(&StanleyADRC::receiveGlobalOdom, this, std::placeholders::_1)); 
    subLocalOdom_ = this->create_subscription<nav_msgs::msg::Odometry>(
      "/odometry/local_filtered", qos, std::bind(&StanleyADRC::receiveLocalOdom, this, std::placeholders::_1)); 
    subPtReport_ = this->create_subscription<deep_orange_msgs::msg::PtReport>(
      "/raptor_dbw_interface/pt_report", 1,
      std::bind(&StanleyADRC::receivePtReport, this, std::placeholders::_1));
    

    // Create Timer
    //rclcpp::Clock ros_clock(RCL_ROS_TIME);
    control_timer_ = rclcpp::create_timer(this, this->get_clock(), std::chrono::milliseconds(static_cast<int>(dt * 1000)), 
                                          std::bind(&StanleyADRC::controlCallback, this));
    ...
    this->max_curv_velocity_message.origin = "curvature limited veloicity - ferdinand controller";
 ...
 private:
  ...
  std::tuple<int, double> findLookaheadIndex(std::vector<geometry_msgs::msg::PoseStamped> refPath, double desLookaheadValue);
  void receiveWheelVelocity(const raptor_dbw_msgs::msg::WheelSpeedReport::SharedPtr msg);
  void receiveVelocity(const std_msgs::msg::Float32::SharedPtr msg);
  void receiveGlobalOdom(const nav_msgs::msg::Odometry::SharedPtr msg);
  void receiveLocalOdom(const nav_msgs::msg::Odometry::SharedPtr msg);
  ...
  void pubError(std::string error_reason, int8_t error_severity, float error_lifetime);
  ...
  rclcpp::Publisher<blackandgold_msgs::msg::StanleyADRCDebug>::SharedPtr controlDebugSignals_;
  ...
  rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr pubCurvatureLookaheadMarker_;
  rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr pubHeadingLookaheadMarker_;
  // Subscribers:
  ...
  rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr subGlobalOdom_;
  rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr subLocalOdom_;
  rclcpp::Subscription<std_msgs::msg::Float32>::SharedPtr subVelocity_;
  rclcpp::Subscription<raptor_dbw_msgs::msg::WheelSpeedReport>::SharedPtr subWheelVelocity_;
  ...
  double yaw_ref_correction = 0.0F;
  double yaw_rate_correction = 0.0F;
  double controller_drive = 0.0F;
  long long unsigned int iteration_counter = 0;
  unsigned int lookahead = 0;
  
  float prev_controller_drive = 0.0F;
  float prev_lat_error = 0.0F;
  float prev_yaw_error = 0.0F;

  bool traction_control = false; // whether we estimate that our rear wheels are sliding

  double curvature_ = 0.0;
  double speed_ = 0.00001;
